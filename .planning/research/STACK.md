...
**Confidence:** HIGH (versions verified against the Go module proxy and GitHub releases on 2026-05-22; architectural patterns cross-validated against gitleaks v8.30.1 and trufflehog source `go.mod` files)

> Prescriptive summary: Build on **Go 1.26**, **cobra** for the CLI, **go-toml/v2** for config (TOML, decoded directly — no viper), shell out to **`git log -p`** parsed by **gitleaks/go-gitdiff** for history (NOT go-git), the **standard library `regexp` (RE2)** for rules with **Shannon entropy + allowlists** to compensate for RE2's lack of lookahead/backreferences, **errgroup + a bounded semaphore** for concurrency, **encoding/json** for output, **goreleaser** for distribution, and **testify** + golden files for tests. This mirrors how gitleaks is built, which is the most directly comparable tool to Mimir.
...
go mod init github.com/<owner>/mimir

# Core
go get github.com/spf13/cobra@v1.10.2
go get github.com/pelletier/go-toml/v2@v2.3.1
go get github.com/gitleaks/go-gitdiff@v0.9.1
...
**If prioritizing the leanest possible binary (per PROJECT.md "stdlib-leaning" constraint):**
...
**If you need to scan bare repos or environments without a `git` binary:**
...
**If false-positive rate becomes the dominant problem (it is the make-or-break factor per PROJECT.md):**
...
**If live verification expands beyond AWS/GitHub later:**
...
Mimir should follow the **gitleaks** column: it is the closest match to Mimir's goals (fast, regex+entropy, single Go binary, CI/pre-commit/baseline). Borrow trufflehog's *verification* idea (scoped to AWS+GitHub) and detect-secrets' *baseline* discipline.

**RE2 constraint handling (explicit):** Go's `regexp` (RE2) does not support lookahead `(?=...)`, lookbehind, or backreferences `\1`. This is intentional (linear-time, ReDoS-safe). The proven mitigations, all used in the field:
1. **Entropy gate** — require a minimum Shannon entropy on the match/capture group to reject low-randomness false positives that a lookahead might otherwise have excluded.
2. **Allowlists / stopwords** — regex patterns (or literal lists) that *exclude* known-benign matches (example keys, placeholders like `AKIAIOSFODNN7EXAMPLE`).
3. **Post-match Go logic** — use capture groups and ordinary Go code to do the "context check" a lookahead would have done (e.g. confirm a nearby keyword, validate a checksum/prefix).

**Entropy approach (explicit):** Use **Shannon entropy** over the candidate string: `H = -Σ p(c)·log2 p(c)` across the distinct bytes/runes, returning a float roughly in `[0, 8]`. Typical thresholds: ~**3.0–3.5 for hex** strings, ~**4.0–4.5 for base64** strings (base64's larger alphabet raises baseline entropy). Make the threshold **per-rule** in the TOML so each signature can tune it, plus a separate generic high-entropy detector for unknown secrets. This is a ~30-line stdlib function (`math.Log2`) — no third-party entropy library needed.
...
---
*Stack research for: Go secret/credential scanner (CLI + CI gate + pre-commit hook)*
*Researched: 2026-05-22*
